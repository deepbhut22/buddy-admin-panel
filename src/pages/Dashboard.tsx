...
interface DashboardData {
  ...
  charts: {
    redemptionsOverTime: Record<string, number>;
    redemptionsByCategory: Array<{ category: string; count: number }>;
    redemptionsByCompany: Array<{ company: string; count: number }>;
    redemptionsByType: Array<{ type: string; count: number }>;
    userRegistrationsOverTime: Record<string, number>;
    usersByCategory: Array<{ category: string; count: number }>;
    usersByService: Array<{ service: string; count: number }>;
    redemptionStatusDistribution: Array<{ status: string; count: number }>;
    monthlyRedemptionData: Array<{ month: string; count: number }>;
  };
  recentData: {
...
interface RedemptionAnalytics {
  timeSeriesData: Array<{ date: string; redemptions: number }>;
...
interface SummaryReports {
  ...
  userStatusCount: Array<{ status: string; count: number }>;
...
export default function AdminDashboard() {
  ...
  const [redemptionAnalytics, setRedemptionAnalytics] = useState<RedemptionAnalytics | null>(null);
  const [userAnalytics, setUserAnalytics] = useState<UserAnalytics | null>(null);
  const [summaryReports, setSummaryReports] = useState<SummaryReports | null>(null);
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [dashboardResponse, redemptionResponse, userResponse, summaryResponse] = await Promise.all([
          axios.get(`http://localhost:5000/api/admin/dashboard?period=${period}`),
          axios.get(`http://localhost:5000/api/admin/redemption-analytics?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`),
          axios.get('http://localhost:5000/api/admin/user-analytics'),
        ...
        const { data: redemptionData } = redemptionResponse;
  ...
  const redemptionsOverTimeData = Object.keys(data.charts.redemptionsOverTime).map(date => ({
    date,
    redemptions: data.charts.redemptionsOverTime[date as keyof typeof data.charts.redemptionsOverTime]
  }));

  const userRegistrationsOverTimeData = Object.keys(data.charts.userRegistrationsOverTime).map(date => ({
    date,
    registrations: data.charts.userRegistrationsOverTime[date as keyof typeof data.charts.userRegistrationsOverTime]
  }));
  ...
  async function downloadRedemptionsData(): Promise<void> {
    try {
      const response = await axios.get('http://localhost:5000/api/admin/export/redemptions', {
      ...
      link.href = url;
      link.setAttribute('download', 'redemptions.xlsx'); // File name
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch (error) {
      alert('Error downloading redemptions data');
      console.error(error);
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button 
                onClick={downloadRedemptionsData}
